...
int main ( int argc, char** argv ) {
    ...
    MPI_File_open(MPI_COMM_WORLD, "version2_output.csv", MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &fh);
    ...
    for (int i = iter_first; i < iter_last; i++) {
        ...
        if (i == 0) {
            clock_t wt_end = clock();
            write_time += (double) (wt_end - wt_begin) / CLOCKS_PER_SEC;
            printf("First value written in %f\n", write_time);
        }
